	...
	<div id="p" class="easyui-panel" title="商品修改"
		...
		<form method="post" action="/sport/AddGoodsServlet">
			<input type="hidden" name="sid" value="1" /> 商品名称：<input type="text"
				name="name" /><br> <br> 分&nbsp;&nbsp;类：<select
				name="catid"><c:forEach var="cate" items="${cates }">
					<option value="${cate.catid }"><c:out
							value="${cate.name }"></c:out></option>
				</c:forEach></select><br> <br> 品&nbsp;&nbsp;牌：<input type="text" name="brand" /><br>
			<br> 价&nbsp;&nbsp;格：<input type="text" name="price" /><br>
			<br> 总&nbsp;&nbsp;数：<input type="text" name="amount" /><br>
			<br> 销&nbsp;&nbsp;量：<input type="text" name="sales" /><br>
			<br> 人&nbsp;&nbsp;气：<input type="text" name="popularity" /><br>
			<br> 描&nbsp;&nbsp;述：<input type="text" name="des" /><br> <br>
			图片路径：<input type="text" name="imgpath" /><br> <br>
			活&nbsp;&nbsp;动：<select name="actid"><c:forEach
